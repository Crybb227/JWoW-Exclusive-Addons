...
local function currentPlayerZoneKey()
    local zoneName = nil
...
function NT:EnsureDisplayedZone()
    ...
    if (not self.db or self.db.preferCurrentZoneMap ~= false) and not self.data.displayedZoneKey then
        local playerZoneKey = currentPlayerZoneKey()
        if playerZoneKey then
            for _, zone in ipairs(zones) do
                if zone.zoneKey == playerZoneKey then
