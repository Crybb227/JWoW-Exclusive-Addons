        ...
        if isNemesisInZone(nemesis, displayedZoneId, displayedZoneName, displayedZoneKey) then
            ...
            if not marker then
                ...
                marker:SetScript("OnClick", function(button, mouseButton)
                    ...
                    if mouseButton == "RightButton" and NT.SetNemesisWaypoint then
                        NT:SetNemesisWaypoint(NT.data.nemeses[button.spawnId])
                    end
                end)
                ...
                marker:SetScript("OnEnter", function(button)
                    ...
                    if NT.AffixUI and type(NT.AffixUI.AddNemesisTooltipLines) == "function" then
                        NT.AffixUI:AddNemesisTooltipLines(GameTooltip, target)
                    end
                    GameTooltip:AddLine("Right-click: Set TomTom waypoint", 0.3, 0.8, 1.0)
                    GameTooltip:Show()
                end)
...
function UI:CreateRow(parent, index)
    ...
    row:SetScript("OnDoubleClick", function(button)
        ...
        if NT.SetNemesisWaypoint then
            NT:SetNemesisWaypoint(button.nemesis)
        end
    end)
    ...
    row:SetScript("OnEnter", function(button)
        ...
        if NT.AffixUI and type(NT.AffixUI.AddNemesisTooltipLines) == "function" then
            NT.AffixUI:AddNemesisTooltipLines(GameTooltip, nemesis)
        end
        GameTooltip:Show()
    end)
